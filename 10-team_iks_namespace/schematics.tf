...
data "ibm_schematics_workspace" "db_storage_workspace" {
  workspace_id = var.db_ws_id
}

data "ibm_schematics_output" "db_workspace" {
  workspace_id = var.db_ws_id
  template_id  = data.ibm_schematics_workspace.db_storage_workspace.template_id[0]
}
...
data "ibm_schematics_output" "groups_workspace" {
  workspace_id = var.groups_ws_id
