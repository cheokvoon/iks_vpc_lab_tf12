...
resource "ibm_container_vpc_cluster" "cluster" {
  ...
  zones {
    subnet_id = lookup(data.ibm_schematics_output.vpc_workspace.output_values,"subnet_ids.0")
    #subnet_id = "${data.ibm_schematics_output.vpc_workspace.output_values.subnet_ids[0]}"
    name      = "${var.ibm_region}-1"
  }
  zones {
    subnet_id = lookup(data.ibm_schematics_output.vpc_workspace.output_values,"subnet_ids.1")
    #subnet_id = "${data.ibm_schematics_output.vpc_workspace.output_values.subnet_ids[1]}"
    name      = "${var.ibm_region}-2"
  }
  zones {
    subnet_id = lookup(data.ibm_schematics_output.vpc_workspace.output_values,"subnet_ids.2")
